...
import StarPlusComp from '../components/StarPlusComp';
import { motion } from 'framer-motion';
import { useRef } from 'react';

const PopularInTalkShow = () => {
  const starPlus = useFirebase();
  ...
  useEffect(() => {
    ...
    if (starPlus.plusData) {
      starPlus.plusData.forEach((data) => {
        arr.push(data.data())
      });
      setUser(arr)
    }
    ...
    <>
      <Container>
        <span style={{
          ...
          marginBottom: '1em',
          marginTop: '.8em'
        }}>Popular in Talk Shows</span>
        <StarPlusComp />
      </Container>
      <MoboRes>
        <div>
          <span>Popular in Talk Shows</span>
          <span>MORE</span>
        </div>
...
`;
export default PopularInTalkShow;
